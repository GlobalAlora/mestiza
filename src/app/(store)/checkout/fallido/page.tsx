    ...
    <main className="px-4 pb-16">
      <div className="mx-auto max-w-lg py-16 text-center">
        ...
        <h1 className="text-ink mb-2 text-2xl font-semibold">
          {c(content, 'fallido.title', 'El pago no se pudo procesar')}
        </h1>
        {orderNumber && (
          <p className="text-muted mb-4 text-sm">
            Pedido <strong className="text-ink">#{orderNumber}</strong> —{' '}
            {c(content, 'fallido.order_suffix', 'el pago fue rechazado o cancelado.')}
          </p>
        )}
